...
ps = (
    ...
    "Where-Object { $_.CommandLine -match 'watchdog-dsh' -and $_.CommandLine -notmatch '-Command' } | "
    "Select-Object -ExpandProperty ProcessId"
)
wd = run(["pwsh", "-NoProfile", "-Command", ps])
wd_pids = [int(x) for x in wd.stdout.split()] if wd.returncode == 0 else []
...
# 复活：WMI 拉起（父 WmiPrvSE，不受 Job 连带——与启动链 Ensure-Watchdog 同款）
if not wd_pids:
    r = run(["pwsh", "-NoProfile", "-Command",
             ...
             "@{CommandLine=\"`\"$pwsh`\" -NoProfile -ExecutionPolicy Bypass -File `\"$o\\watchdog-dsh.ps1`\"\";CurrentDirectory=$o} "
    ...
    if r.stdout.strip() == "0":
        ...
        fail(f"看门狗 WMI 复活失败 (ReturnValue {r.stdout.strip() or r.returncode})")
