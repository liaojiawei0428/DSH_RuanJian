...
.dshgp-item { display:flex; align-items:flex-start; gap:10px; padding:9px 12px; min-width:0;
  border:1px solid var(--dsw-alias-border-l1,rgba(0,0,0,0.04)); border-radius:12px;
...
.dshgp-dot { width:8px; height:8px; border-radius:50%; flex:none; margin-top:8px;
  background:var(--dsw-alias-label-tertiary,#81858c); }
...
.dshgp-push input { height:32px; padding:0 10px; font-size:13px;
  border:1px solid var(--dsw-alias-border-l2,rgba(0,0,0,0.1)); border-radius:8px; outline:none;
  background:var(--dsw-alias-bg-layer-1,#fff); color:var(--dsw-alias-label-primary,#0f1115);
  font-family:var(--dsw-font-family,-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC",sans-serif);
  width:100%; box-sizing:border-box; }
...
function GhEntry(props) {
  ...
  useEffect(() => {
    ...
    const probe = async () => {
      try {
        ...
        if (alive && data?.bindings !== undefined) setBindingCount(data.bindings.length)
  ...
  return [
    h('button', {
      ...
      h('span', { className: 'dot' + (bindingCount > 0 ? ' on' : '') }),
      'Git 推送'),
    open ? h(GhManager, {
      key: 'panel',
      call: props.call,
...
/**
 * Modal: binding roster + add/edit form + manual push.
 * @param {{call: Function, onClose: () => void}} props
 */
        ...
        h('div', { className: 'dshgp-body' },
          ...
          h('div', { className: 'dshgp-bound' },
            h('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between' } },
              h('p', { className: 'dshgp-pane-title', style: { margin: 0 } }, '网络设置'),
            ),
            h('div', { className: 'dshgp-proxy-row' },
              ...
              h('button', {
                ...
                onClick: () => {
                  void act('settings.set', { proxy: proxyInput.trim() }, () => setNotice({ kind: 'ok', text: '代理设置已保存' }))
                },
              }, '保存'),
            ),
          ),
          ...
          pushing !== undefined && (() => {
            ...
            return h('div', { className: 'dshgp-push' },
              h('div', { className: 'dshgp-name' }, `推送「${b.name}」到 ${repoLabel(b.repoOwner, b.repoName)}（分支 ${b.branch}）`),
